...
export type DefaultObjectWithOffset = {
    offset: number;
    next_offset: number;
    data: any[];
    total: number;
...
export function readWithOffset(
    source: any[] | Results<RealmObject<DefaultObject, never> & DefaultObject>,
    offset: number,
    limit: number,
): DefaultObjectWithOffset {
    ...
    return {
        ...
        total: source.length,
        limit: limit,
        next_offset: Math.min(offset + s.length, source.length),
    };
}

export async function readAllWithOffset(
    callable: DefaultFunctionWithOffset,
    offset: DefaultObjectWithOffset,
): Promise<DefaultObjectWithOffset> {
    let limit = offset.limit,
