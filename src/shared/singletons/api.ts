...
export default class Api extends SingletonBase {
    ...
    async save_cached_beatmaps(user_id: number, beatmap_ids: number[]) {
        if (this.cached_beatmaps.size > 0) {
            const maps = Array.from(this.cached_beatmaps.values()).filter((map) =>
                beatmap_ids.includes(map.beatmap_id),
            );
    ...
    async get_cached_beatmaps(user_id: number) {
        this.cached_beatmaps.clear();
        const maps = await readJsonV2<RespUserMostPlayedBeatmap[]>(
            ServiceCode.Data,
            SpaceId.Beatmaps,
            `1.${user_id}.json`,
        );
    ...
    async get_pack(tag_id: any): Promise<CachedBeatmapPack> {
        ...
        if (this.cached_packs.size === 0) {
            ...
            if (packs !== null) {
                packs.forEach((pack) => {
                    this.cached_packs.set(pack.tag, pack);
                });
            }
        }
        const cached_packs = this.cached_packs.get(tag_id);
        if (cached_packs) return { cached: true, ...cached_packs };
    ...
    async lookup_beatmap_by_checksum(checksum: string) {
        if (this._client === undefined) {
            throw new Error("API client has not been initialized.");
        }
        return this._client.r_beatmaps.lookup({ checksum });
    }
}
