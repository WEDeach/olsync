...
export default class Controller {
    ...
    handleSelectFile = async () => {
        const result = await window.olsCore.openFile("Select directory for osu!stable", "openDirectory", []);
        if (result === undefined) return;
        await g.saveConfig(ConfigKey.PATH_STABLE_DIR, result);
    };

    onBtnPathPatchClicked = async () => {
        const result = await window.olsCore.exec("osustable.File.osz");
        runInAction(() => {
    ...
    onBtnPathPatchV2Clicked = async () => {
        const result = await window.olsCore.exec("osu!");
        runInAction(() => {
            this.state.path_patch = result;
        });
    };
    ...
    onBtnReadScoresClicked = async () => {
        await window.olsCore.initStableReader(this.Path);
        const result = await window.olsCore.getScores(OsuClients.Stable);
        console.log(result);
    };
        ...
        try {
            ...
            switch (currentSchema) {
                case OsuTables.BEATMAP_COLLECTION.toString(): {
                    const result = await window.olsCore.getCollections();
