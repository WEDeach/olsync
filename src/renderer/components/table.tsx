...
export const Table = observer(
    ({ dimensions, heads, rows, onScrollToBottom, threshold = 10, rowHeight = 100 }: Props) => {
        ...
        return (
            <TableContainer sx={{ height: height, width: width }} component={Paper} onScroll={onScroll}>
                <MTable stickyHeader>
                    <TableHead>
                        <TableRow>
                            {heads.map((property) => (
                                <TableCell key={property.name}>
                                    {property.checkbox_toggler && (
                                        <FormControlLabel
                                            ...
                                            onChange={() =>
                                                property.checkbox_toggler?.callback(!property.checkbox_toggler?.checked)
                                            }
                                        />
                                    )}
                                    {property.name}
                    ...
                    <TableBody>
                        {visibleRange.startIndex > 0 && (
                            <TableRow style={{ height: visibleRange.startIndex * rowHeight }}>
                                <TableCell colSpan={heads.length} sx={{ border: 0, padding: 0 }} />
                            </TableRow>
                        )}
                        ...
                        {visibleRows.map((row, index) => {
                            ...
                            return (
                                ...
                                >
                                    {heads.map((property) => (
                                        <TableCell key={property.name}>{renderCellContent(row, property)}</TableCell>
                                    ))}
                                </TableRow>
                            );
                        })}
...
const renderCellContent = (row: any, property: IPropertyWithName) => {
    ...
    if (property.type === "uuid") {
        ...
        ["string", "bool", "int", "float", "double", "decimal128", "objectId", "data", "date", "mixed"].includes(
            property.type,
        )
    ) {
        r = row[key];
        ...
        const url = row[key];
        return <Avatar alt="" src={url} variant="square" />;
